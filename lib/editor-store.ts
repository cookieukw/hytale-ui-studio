...
import type { HytaleComponent, EditorState, ViewMode, DevicePreview } from './hytale-types'
...
function addComponentToParent(
  ...
  return components.map((comp) => {
    ...
    if (comp.children) {
      return {
        ...comp,
        children: addComponentToParent(comp.children, parentId, newComponent, index),
      }
    }
    return comp
  })
...
function componentsToCode(components: HytaleComponent[], indent = 0): string {
  ...
  for (const comp of components) {
    ...
    if (comp.placeholderText) code += `${spaces}  PlaceholderText: "${comp.placeholderText}"\n`
    ...
    if (comp.anchor) {
      if (comp.anchor.full) {
        ...
        if (comp.anchor.width) code += `${spaces}  Anchor.Width: ${comp.anchor.width}\n`
        if (comp.anchor.height) code += `${spaces}  Anchor.Height: ${comp.anchor.height}\n`
        if (comp.anchor.top !== undefined) code += `${spaces}  Anchor.Top: ${comp.anchor.top}\n`
        if (comp.anchor.bottom !== undefined) code += `${spaces}  Anchor.Bottom: ${comp.anchor.bottom}\n`
        if (comp.anchor.left !== undefined) code += `${spaces}  Anchor.Left: ${comp.anchor.left}\n`
        if (comp.anchor.right !== undefined) code += `${spaces}  Anchor.Right: ${comp.anchor.right}\n`
      }
    }
    ...
    // Padding
    if (comp.padding) {
      if (comp.padding.top) code += `${spaces}  Padding.Top: ${comp.padding.top}\n`
      if (comp.padding.bottom) code += `${spaces}  Padding.Bottom: ${comp.padding.bottom}\n`
      if (comp.padding.left) code += `${spaces}  Padding.Left: ${comp.padding.left}\n`
      if (comp.padding.right) code += `${spaces}  Padding.Right: ${comp.padding.right}\n`
    }
    
    // Background
    if (comp.background) {
      if (comp.background.color) code += `${spaces}  Background.Color: "${comp.background.color}"\n`
      if (comp.background.border) code += `${spaces}  Background.Border: "${comp.background.border}"\n`
      if (comp.background.opacity !== undefined) code += `${spaces}  Background.Opacity: ${comp.background.opacity}\n`
    }
    
    // Text Style
    if (comp.textStyle) {
      if (comp.textStyle.fontSize) code += `${spaces}  FontSize: ${comp.textStyle.fontSize}\n`
      if (comp.textStyle.renderBold) code += `${spaces}  RenderBold: true\n`
      if (comp.textStyle.renderUppercase) code += `${spaces}  RenderUppercase: true\n`
      if (comp.textStyle.textColor) code += `${spaces}  TextColor: "${comp.textStyle.textColor}"\n`
      if (comp.textStyle.alignment) code += `${spaces}  Alignment: ${comp.textStyle.alignment}\n`
    }
...
interface EditorStore extends EditorState {
  ...
  // Components
  addComponent: (component: Omit<HytaleComponent, 'id'>, parentId?: string | null, index?: number) => string
...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  addComponent: (component, parentId = null, index) => {
    ...
    set((state) => ({
      components: addComponentToParent(state.components, parentId, newComponent, index),
      selectedId: id,
    }))
  ...
  duplicateComponent: (id) => {
    ...
    const findParentAndIndex = (
      ...
      for (let i = 0; i < components.length; i++) {
        ...
        if (components[i].children) {
          const result = findParentAndIndex(components[i].children!, targetId, components[i].id)
    ...
    if (location) {
      set((state) => ({
        components: addComponentToParent(state.components, location.parentId, duplicate, location.index),
  ...
  moveComponent: (id, newParentId, index) => {
    ...
    // Add to new position
    newComponents = addComponentToParent(newComponents, newParentId, comp, index)
